...
class SolrService {
    ...
    SolrServer getServer() {
        ...
        if (!servers[url]) {
            ...
            // Set a socket timeout as a workaround for long-query-problems. (see #6825)
            server.soTimeout = 20000

            servers[url] = server
        }
        return servers[url] as SolrServer
    }

    StreamingUpdateSolrServer getStreamingUpdateServer(queueSize = 20, numThreads = 3) {
        def url = (grailsApplication.config.solr?.url) ? grailsApplication.config.solr.url : "http://localhost:8983/solr"
